...
```bash
cd chatbot
python -m venv tft-faq-bot/venv
tft-faq-bot/venv/Scripts/activate
...
```bash
tft-faq-bot/venv/Scripts/uvicorn api.main:app --reload
```
...
| --- | --- | --- |
| `POST` | `/ask` | Submit a question, returns an answer |
...
```json
{ "answer": "Infinity Edge grants Precision and causes..." }
```
...
1. Push repo to GitHub (including `embeddings/chroma_db/`)
2. Create a **Web Service** on [Render](https://render.com), point it at the repo
3. Render auto-detects `render.yaml` for build/start commands
4. Add `GROQ_API_KEY` under **Settings → Environment**

### Frontend — Vercel

1. Import the same repo on [Vercel](https://vercel.com)
2. Set **Root Directory** to `chatbot`, **Output Directory** to `frontend`
